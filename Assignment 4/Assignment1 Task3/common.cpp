...
bool select_one_device(cl::Platform* platfm, cl::Device* dev){
	...
	try {
		...
		//check if valid option selected and if input was integer
		if (stringStream >> selected) {
			char c;
			if (!(stringStream >> c)) {
				if (selected >= 0 && selected < optionCounter) {
					int platformNumber = options[selected].first;
					int deviceNumber = options[selected].second;
